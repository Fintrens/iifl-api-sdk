...
public class FintrensRequestHandler {
	...
	String processPostHttpRequest(HttpPost request,JSONObject data, String  requestname,String authToken){
		...
		try {
			...
			// TODO Auto-generated catch block
			logger.info("{} failed due to IOException : {} for userId: {}", requestname, e.getMessage(), data.get("userId"));
		} catch (APIException e) {
			// TODO Auto-generated catch block
			logger.info("{} failed due to APIException: {} for userId: {}", requestname, e.getMessage(), data.get("userId"));
		}
		return content;
